...
const { getJobData, getJobSchema, TOTAL_JOBS, jobTitles, companies, spainLocations, industries } = require('./jobData');
...
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Helvetica Neue',sans-serif;background:#f5f5f5;color:#222;line-height:1.6}
a{color:inherit;text-decoration:none}
/* NAV */
nav{background:#c60b1e;color:#fff;padding:0 1.5rem;display:flex;align-items:center;justify-content:space-between;height:60px;position:sticky;top:0;z-index:100}
...
/* HERO */
.hero{background:linear-gradient(135deg,#c60b1e 0%,#8b0815 50%,#5a050e 100%);color:#fff;padding:3rem 1.5rem;text-align:center}
...
.search-bar input:focus,.search-bar select:focus{border-color:#c60b1e}
...
.filter-chip.active,.filter-chip:hover{background:#c60b1e;color:#fff;border-color:#c60b1e}
/* LAYOUT */
...
.job-card:hover{border-color:#c60b1e;transform:translateY(-2px)}
...
.btn-apply{padding:.55rem 1.3rem;background:#c60b1e;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:.85rem;cursor:pointer;transition:background .2s}
.btn-apply:hover{background:#8b0815}
/* JOB DETAIL */
...
.btn-apply-big{padding:.85rem 2.5rem;background:#c60b1e;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:1rem;cursor:pointer;transition:background .2s}
.btn-apply-big:hover{background:#8b0815}
/* PAGINATION */
...
.pagination a:hover{border-color:#c60b1e;color:#c60b1e}
.pagination .current{background:#c60b1e;color:#fff;border-color:#c60b1e}
/* BREADCRUMB */
.breadcrumb{font-size:.82rem;color:#888;margin-bottom:1rem}
.breadcrumb a{color:#c60b1e}
/* SITEMAP NOTE */
.info-box{background:#fff;border-radius:12px;padding:1.25rem 1.5rem;border-left:4px solid #c60b1e;margin-bottom:1rem;font-size:.88rem}
/* FOOTER */
...
.modal input:focus{border-color:#c60b1e}
.modal .btn-submit{width:100%;padding:.75rem;background:#c60b1e;color:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:.95rem}
.modal .btn-submit:hover{background:#8b0815}
...
${AD_TOP}
<nav>
  <a class="brand" href="/"><span>ES</span>Jobs<span>.es</span></a>
  <div class="nav-links">
    <a href="/">Home</a>
...
${AD_BOTTOM}
<footer>
  &copy; 2025 ESJobs.es — <strong>100,000 Jobs</strong> across Spain |
...
<div class="job-card">
  ...
  const websiteSchema = {
    ...
    "name": "ESJobs.es",
    "url": baseUrl,
    "description": "Spain's largest job portal with 100,000 job listings — remote and on-site across all regions",
    "potentialAction": {
      "@type": "SearchAction",
  ...
  const body = `
<div class="hero">
  <h1>Find Your Dream Job in <span class="accent">Spain</span></h1>
  <p>100,000 verified job listings — remote & on-site — across all 17 autonomous communities</p>
  ...
  <div class="stat-bar">
    ...
    <div class="stat"><strong>17</strong><span>Autonomous Communities</span></div>
...
<div class="container">
  <div class="info-box">
    🇪🇸 Spain's most comprehensive job board — browse <strong>50,000 remote jobs</strong> and <strong>50,000 on-site jobs</strong> across all industries.
  </div>
   ${AD_MIDDLE}
  ...
  res.send(renderHTML({
    title: 'ESJobs.es — 100,000 Jobs in Spain | Remote & On-site',
    meta: 'Find your next job in Spain. 100,000 verified listings — 50,000 remote and 50,000 on-site jobs across all 17 autonomous communities.',
    bodyContent: body,
...
<div class="hero" style="padding:1.75rem 1.5rem">
  <h1 style="font-size:1.8rem">Browse <span class="accent">100,000 Jobs</span> in Spain</h1>
...
<div class="container">
  ...
  res.send(renderHTML({
    title: `Spain Jobs — Page ${page} of ${totalPages.toLocaleString()} | ESJobs.es`,
    meta: `Browse ${TOTAL_JOBS.toLocaleString()} jobs in Spain. Page ${page}. Remote and on-site positions across all industries.`,
    bodyContent: body,
    schema: null,
    req
  }));
...
app.get('/jobs/:id', (req, res) => {
  ...
  if (!id || id < 1 || id > TOTAL_JOBS) {
    return res.status(404).send(renderHTML({
      title: 'Job Not Found | ESJobs.es',
      meta: 'This job listing was not found.',
      bodyContent: `<div class="container" style="text-align:center;padding:4rem 1.5rem"><h1>404 — Job Not Found</h1><p style="margin:1rem 0 2rem">This job may have been filled or removed.</p><a href="/jobs" style="color:#c60b1e">← Browse All Jobs</a></div>`,
      schema: null,
      req
    }));
  }
...
<div class="container">
  ...
  <div class="job-detail">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.5rem">
        ...
        <span style="font-size:.8rem;color:#888">Job ID: ES-${String(job.id).padStart(6, '0')}</span>
      </div>
    </div>
  ...
  <div style="text-align:center;margin-top:1.5rem">
    <a href="/jobs" style="color:#c60b1e;font-weight:600">← Browse All 100,000 Jobs</a>
  </div>
</div>`;

  res.send(renderHTML({
    title: `${job.title} at ${job.company} — ${job.location} | ESJobs.es`,
    meta: `${job.title} job at ${job.company}. ${job.isRemote ? 'Remote' : job.location}. ${job.salary}. Apply now on ESJobs.es.`,
    bodyContent: body,
    schema,
    req
  }));
...
app.get('/sitemap', (req, res) => {
  ...
  const body = `
<div class="container">
  <h1 style="margin-bottom:1rem">Sitemap — ESJobs.es</h1>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-top:1rem">
    <div class="job-card">
      ...
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/" style="color:#c60b1e">🏠 Home</a>
        <a href="/jobs" style="color:#c60b1e">📋 All Jobs (100,000)</a>
        <a href="/jobs?type=remote" style="color:#c60b1e">🌐 Remote Jobs (50,000)</a>
        <a href="/jobs?type=onsite" style="color:#c60b1e">🏢 On-site Jobs (50,000)</a>
      </div>
    </div>
    <div class="job-card">
      <div class="card-title">XML Sitemaps</div>
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/sitemap.xml" style="color:#c60b1e">📄 Sitemap Index</a>
        <a href="/sitemap-1.xml" style="color:#c60b1e">📄 Sitemap 1 (Jobs 1–1,000)</a>
        <a href="/sitemap-2.xml" style="color:#c60b1e">📄 Sitemap 2 (Jobs 1,001–2,000)</a>
    ...
    <div class="job-card">
      ...
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/jobs/1" style="color:#c60b1e">Job #1 (First Remote Job)</a>
        <a href="/jobs/50000" style="color:#c60b1e">Job #50,000 (Last Remote Job)</a>
        <a href="/jobs/50001" style="color:#c60b1e">Job #50,001 (First On-site Job)</a>
        <a href="/jobs/100000" style="color:#c60b1e">Job #100,000 (Last On-site Job)</a>
      </div>
    </div>
  </div>
</div>`;
  ...
  res.send(renderHTML({
    title: 'Sitemap | ESJobs.es',
    meta: 'Complete sitemap of ESJobs.es with 100,000 job listings across Spain.',
    bodyContent: body,
    schema: null,
    req
  }));
...
app.listen(PORT, () => {
  console.log(`🇪🇸 ESJobs.es running on port ${PORT}`);
  console.log(`📋 ${TOTAL_JOBS.toLocaleString()} job pages ready`);
  console.log(`🏢 ${companies.length} companies hiring in Spain`);
  console.log(`📍 ${spainLocations.length} locations across Spain`);
});
